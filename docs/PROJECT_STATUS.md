...
> **2026-09-15 development checkpoint（M1-1..M1-4）**：M1-1 scaffold、M1-2 material/section、M1-3a rigid platens/control nodes/rigid body、M1-3b boundary conditions、M1-4 contact 均已完成。`pipeline/physical_builder.py` + `build-physical` CLI 已用真实 Fig.1 mesh bundle 多次冒烟通过（`work/fig1_build_checkpoint_001`、`work/fig1_build_m13a_smoke_001`、`work/fig1_build_m13b_smoke_001`、`work/fig1_build_m14_smoke_001` 等），当前生成 `ingredients/`、`blocks/material_section.inc`、`blocks/rigid_platens.inc`、`blocks/boundary_conditions.inc`、`blocks/contact.inc` 与 `model_manifest.json`（`status=PHYSICAL_BUILD_PARTIAL`，`dataset_eligible=false`，无 physical.inp）。以上仅为 **Python builder + real Fig.1 mesh bundle smoke**；**Abaqus Physical Data Check 未执行、自动 solve 未执行**，不构成任何 Abaqus 验证。Pixi 下 58/58 测试通过。早期 checkpoint（环境迁移、Fig.1 网格回归、M1-1..M1-3b 细节）见 git 历史与 IMPLEMENTATION_STATUS。以下 2026-09-13 接管核查记录保持原样。
