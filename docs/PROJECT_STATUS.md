...
> **2026-09-15 development checkpoint（M1-1..M1-7 DONE；M2 DONE, COMPLETED_WITH_WARNINGS；M3 DONE, COMPLETED_WITH_WARNINGS）**：M1 BUILD complete（M1-1..M1-7），`pipeline/physical_builder.py` + `build-physical` 生成 `ingredients/`、7 个 blocks、顶层 `physical.inp`（9 条固定顺序相对路径 `*Include`，原子发布）、`build_report.json`（13 项 static checks）与 `model_manifest.json`（`status=PHYSICAL_INP_STATIC_VALIDATED`，`dataset_eligible=false`）。**M2 完成：`pipeline/physical_datacheck.py` + CLI `datacheck` 已把 Fig.1 自动 `physical.inp` 交给真实 Abaqus 2026 Physical Data Check 并通过**（`work/fig1_datacheck_m2_final_001`：returncode=0、`ANALYSIS DATACHECK COMPLETE`、0 error、10 warnings 全部保留 → `status=DATACHECK_COMPLETED_WITH_WARNINGS`）。execution policy（`cpus=1, standard_parallel=solver`，safe Data Check profile）是 machine/runtime-local 配置（CLI → `config/datacheck_runtime.local.json` → safe default），只进命令行与 datacheck provenance，永不进入 physical.inp 或 M1 identity keys。**M3 完成：`pipeline/physical_solve.py` + CLI `solve` 已对通过 M2 的 20% validation deck（`work/fig1_build_m3_20pct_001`，唯一差异 strain 0.3→0.2 的 local config）自动执行真实 Abaqus/Standard analysis**（`work/fig1_solve_m3_20pct_003`：returncode=0、`Abaqus JOB fig1_m3_solve COMPLETED`、`.sta: THE ANALYSIS HAS COMPLETED SUCCESSFULLY`、last_increment=59/last_step_time=1.00=目标、0 error、17 warnings 保留、ODB artifact 16,075,836 bytes → `status=SOLVE_COMPLETED_WITH_WARNINGS`；solve runtime profile：`cpus=4, standard_parallel=solver`，machine-local `solve_runtime.local.json`；M3.1 hardening：M2 completion marker gate、job-specific stdout token、non-empty ODB gate、`execution.wall_time_s`）。**Warning 语义**：solve 的 17 条 = 10 条 M2 preprocessing warnings 原样重现（signature-identical）+ 7 条 solve-only `.msg` zero-moment warnings；provenance 的 10 条不并入 17；全部未白名单，移交 M4 QA。Pixi 下 **151 core + 8 boundary 测试通过**（2026-09-16 maintenance checkpoint 实测：`pixi run test` → core `Ran 151 tests ... OK`、boundary `Ran 8 tests ... OK`）。**测试数量只与"该次验证 checkpoint"绑定，不再当作长期当前值**；历史 milestone 数量（115 / 138）保留在 `docs/IMPLEMENTATION_STATUS.md` 并标注 `[HISTORICAL]`。**M4 ODB extraction = NOT STARTED（M3 未读取/提取 ODB）；10 条 M2 warning 与 17 条 solve warning 为 M4 mechanics/contact QA 强制输入；`dataset_eligible=false` 恒定。**早期 checkpoint 见 git 历史与 IMPLEMENTATION_STATUS。

核查日期：2026-09-13。范围：工程接管、源资料核对、环境检查、原有轻量测试、现有 ODB 只读检查。未启动任何新 Abaqus Data Check、完整求解或批量计算。

> **[当前状态补充 2026-09-16 · v2 架构施工]**：在上述 checkpoint 之后已完成 v2 Phase 0/1（Fig.1 20% regression freeze + 契约/完整性门 hardening）、**v2 Phase 2（配置语义与 identity 重构）** 与 **v2 Phase 3A（runtime / staging 单真值）**。当前实测：`pixi run test` → core **282 OK** + boundary **8 OK**；`pixi run check` / `plan` / `cli-help` exit 0；Fig.1 20% **byte regression PASS**（`physical.inp` + 9 include 与冻结 SHA 逐字节一致）+ **semantic regression PASS**，且 `physical_model_id` / `solver_spec_id` / `output_spec_id` / `build_artifact_id` 在三轮之间保持完全一致。配置一律经 严格解析 → legacy 归一化（兼容断言 / `NOT_IMPLEMENTED`）→ frozen Normalized Spec 再交给 renderer；identity 拆分为 `physical_model_id` / `solver_spec_id` / `output_spec_id` / `build_artifact_id`（`model_key`/`scaffold_key`/`build_key` 保留为 alias）；执行层统一为 **structured artifact manifest + 单一 staging planner**（deck + provenance 两级 SHA 证明，`Build == DataCheck == Solve` 身份链），runtime 解析与 job 命名各只有一处实现（job name 由 `<case_id>_dc` / `<case_id>_solve` 派生，`--job-name` 可选覆盖）。**这三轮均未运行 Abaqus**，因此 M2/M3 的真实 Abaqus 验证状态仍以 2026-09-15 checkpoint 为准；M4 ODB extraction 仍未启动。详见 `docs/HANDOFF_CURRENT.md` §3b/§3c/§3d。

> **[2026-09-16 · 30 曲面阶段性实验]**：在上述 M3 checkpoint 之后，Pre-M4 consolidation 已完成（commit 852f276），并完成 30 曲面阶段性验证实验（2026-09-15/16，local experiment）：**30 进入 / 28 mesh PASS / 28 build PASS / 28 Data Check accepted / 0 Data Check failed；4 个 solve 尝试（ref30_02/03/04/05）全部未完成、0 个有效 ODB**（ref30_02/04 收敛停滞人工终止、ref30_03 TOO MANY ATTEMPTS、ref30_05 被 Windows Update 强制重启中断于 ≈0.979）。这证明 mesh → build → Data Check 已在一组明显不同曲面上初步泛化；batch/recovery framework 与 solve 收敛性研究仍属后续里程碑。详情见 `docs/EXPERIMENT_30_SURFACES_20260915.md`。M4 仍未启动。
