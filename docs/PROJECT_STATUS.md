...
> **2026-09-15 development checkpoint（M1-1..M1-7 DONE；M2 DONE, COMPLETED_WITH_WARNINGS）**：M1 BUILD complete（M1-1..M1-7），`pipeline/physical_builder.py` + `build-physical` 生成 `ingredients/`、7 个 blocks、顶层 `physical.inp`（9 条固定顺序相对路径 `*Include`，原子发布）、`build_report.json`（13 项 static checks）与 `model_manifest.json`（`status=PHYSICAL_INP_STATIC_VALIDATED`，`dataset_eligible=false`）。**M2 完成：`pipeline/physical_datacheck.py` + CLI `datacheck` 已把 Fig.1 自动 `physical.inp` 交给真实 Abaqus 2026 Physical Data Check 并通过**（`work/fig1_datacheck_m2_final_001`：returncode=0、`ANALYSIS DATACHECK COMPLETE`、0 error、10 warnings 全部保留 → `status=DATACHECK_COMPLETED_WITH_WARNINGS`）。execution policy（`cpus=1, standard_parallel=solver`，safe Data Check profile）是 machine/runtime-local 配置（CLI → `config/datacheck_runtime.local.json` → safe default），只进命令行与 datacheck provenance，永不进入 physical.inp 或 M1 identity keys；开发机 `standard_parallel=all` 在 General Contact preprocessing 存在可复现的 threads/domain 失败（手工 baseline 同样失败，属 reproducible machine/runtime-specific failure；10-attempt 诊断链见 `work/fig1_datacheck_m2_diagnostics_summary.json`），其他机器不得自动假设需要同样 workaround。Pixi 下 **115/115 core + 8/8 boundary 测试通过**。**M3 Solve = NOT STARTED（其 execution policy = TO BE VALIDATED，候选 `cpus=4, standard_parallel=solver`）；10 条 M2 warning 为 M3/M4 QA 强制输入；`dataset_eligible=false` 恒定。**早期 checkpoint（环境迁移、Fig.1 网格回归、M1 细节、2026-09-13 接管核查记录）见 git 历史与 IMPLEMENTATION_STATUS。
